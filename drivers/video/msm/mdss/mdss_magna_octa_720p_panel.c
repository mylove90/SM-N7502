...
#if defined(HBM_RE)
void  mipi_samsung_read_hbm(struct mdss_panel_data *pdata);
#endif
...
#elif defined(TEMPERATURE_ELVSS)
// ELVSS TEMPERATURE COMPENSATION for Magna EA8061
static struct dsi_cmd get_elvss_tempcompen_control_set(void)
{
	struct dsi_cmd elvss_tempcompen_control = {0,};
	...
	/* Get the command desc */
	if (msd.dstat.temperature >= 0)
		elvss_lowtemp_cmds_list.cmd_desc[4].payload[1] = 0x48;
	else
		elvss_lowtemp_cmds_list.cmd_desc[4].payload[1] = 0x4C;
...
static struct dsi_cmd get_hbm_control_set(void)
{
	...
	mipi_samsung_read_hbm(msd.pdata);    // read hbm
...
#elif defined(TEMPERATURE_ELVSS)
	elvss_compen_control.num_of_cmds,
#endif
...
{
		int i=0;
		char hbm_buffer[21]={0,};
	  static bool hbm_read_check = false;

		if (hbm_read_check)
			return;

		mipi_samsung_disp_send_cmd(PANEL_HBM_READ, false);
		mipi_samsung_read_nv_mem(pdata, &nv_hbm_read_cmds,hbm_buffer);    // Read DBh 1st~21st para
		for (i = 0; i < 21; i++)
			hbm_control_cmds.cmd_desc[1].payload[i + 1] = hbm_buffer[i];

		hbm_read_cmd.cmd_desc[0].payload[1] = 0xD4;
		mipi_samsung_disp_send_cmd(PANEL_HBM_READ, false);
		mipi_samsung_read_nv_mem(pdata, &nv_hbm_read_cmds2, hbm_buffer);    // read D4h 18st para
		default_d4_value = hbm_buffer[0];
		hbm_read_cmd.cmd_desc[0].payload[1] = 0xDB;
		mipi_samsung_disp_send_cmd(PANEL_HBM_READ, false);
		mipi_samsung_read_nv_mem(pdata, &nv_hbm_read_cmds3, hbm_buffer);   // read DBh 34st para
		hbm_control_cmds.cmd_desc[7].payload[18] = hbm_buffer[0];
...
static int mdss_dsi_panel_dimming_init(struct mdss_panel_data *pdata)
{
	...
	if (!msd.dstat.is_smart_dim_loaded) {
		...
		mipi_samsung_read_nv_mem(pdata, &nv_mtp_read_cmds, msd.sdimconf->mtp_buffer);   // read mtp
...
static ssize_t mdss_temperature_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int rc;	rc = snprintf((char *)buf, 40,"-20, -19, 0, 1\n");
