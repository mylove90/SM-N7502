...
#define SHA_PANEL		0x53a890
#define SDC_PANEL		0x538890
int lcd_id;
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	pinfo->lcdc.v_pulse_width = (!rc ? tmp : 2);
		...
		rc = of_property_read_u32(np, "qcom,mdss-dsi-h-pulse-width-sdc", &tmp);
		pinfo->lcdc.h_pulse_width = (!rc ? tmp : 2);
		rc = of_property_read_u32(np, "qcom,mdss-dsi-v-back-porch-sdc", &tmp);
		pinfo->lcdc.v_back_porch = (!rc ? tmp : 6);
		rc = of_property_read_u32(np, "qcom,mdss-dsi-v-front-porch-sdc", &tmp);
		pinfo->lcdc.v_front_porch = (!rc ? tmp : 6);
		rc = of_property_read_u32(np, "qcom,mdss-dsi-v-pulse-width-sdc", &tmp);
		pinfo->lcdc.v_pulse_width = (!rc ? tmp : 2);
	}	
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
		...
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
		"qcom,mdss-dsi-on-command-sdc", "qcom,mdss-dsi-on-command-state");	
	}
