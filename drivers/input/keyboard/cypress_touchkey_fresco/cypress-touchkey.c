...
#elif defined(CONFIG_MACH_HLLTE) ||	defined(CONFIG_MACH_HL3G) || defined(CONFIG_KEYBOARD_CYPRESS_TOUCH_20055)
	...
	const struct i2c_device_id *id)
{
	...
	struct touchkey_i2c *tkey_i2c;
	#if defined(TK_USE_LCDTYPE_CHECK) || defined(CONFIG_KEYBOARD_CYPRESS_TOUCH_20055)
	bool bforced = false;
	#endif
...
*/
	if (is_lcd_attached() == 0) {
		printk("[TKEY] %s : is_lcd_attached()=0 \n",__func__);
		return -EIO;
	}
