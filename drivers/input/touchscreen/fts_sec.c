...
static void hover_no_sleep_enable(void *device_data)
{
	...
	if (info->touch_stopped) {
		...
		set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
		info->cmd_state = CMD_STATUS_NOT_APPLICABLE;
#if 0 // for wakeup time
		if(info->cmd_param[0]==1){
			retry_hover_no_sleep_enable_after_wakeup = 1;
			tsp_debug_info(true, &info->client->dev, "%s: retry_hover_no_sleep_enable_after_wakeup \n", __func__);
		}
#endif
		return;
	}
...
static void glove_mode(void *device_data)
{
	...
	if (info->cmd_param[0] < 0 || info->cmd_param[0] > 1) {
		...
		if (!info->touch_stopped && info->reinit_done) {
			...
			if(info->cmd_param[0]==1){
				retry_glove_mode_after_wakeup = 1;
				tsp_debug_info(true, &info->client->dev, "%s: retry_glove_mode_after_wakeup \n", __func__);
			}
		}
#endif
		snprintf(buff, sizeof(buff), "%s", "OK");
...
static void clear_cover_mode(void *device_data)
{
	...
	if (info->cmd_param[0] < 0 || info->cmd_param[0] > 3) {
		...
		if (!info->touch_stopped && info->reinit_done) {
			...
			if(info->cmd_param[0] > 1){
				retry_clear_cover_mode_after_wakeup = 1;
			}else{
				retry_clear_cover_mode_after_wakeup = 0;
			}
			tsp_debug_info(true, &info->client->dev, "%s: retry_clear_cover_mode_after_wakeup,%d \n", __func__, info->cmd_param[0]);
...
static void boost_level(void *device_data)
{
	...
	if (info->dvfs_boost_mode == DVFS_STAGE_NONE) {
		int retval = set_freq_limit(DVFS_TOUCH_ID, -1);
		if (retval < 0) {
