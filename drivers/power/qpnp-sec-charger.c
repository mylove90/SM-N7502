...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	#if	defined(CONFIG_MACH_S3VE) || defined(CONFIG_MACH_S3VE_CHN_CTC) || defined(CONFIG_MACH_CRATERVE_CHN_CTC) || defined(CONFIG_MACH_CRATERQ)
	mdelay(20);
	#endif
...
#if	defined(CONFIG_MACH_S3VE) || defined(CONFIG_MACH_S3VE_CHN_CTC) || defined(CONFIG_MACH_CRATERVE_CHN_CTC)
...
/* bug-fix to read temperature without battery */
	...
	if (((int)results.physical - previous_temp_value > SUDDEN_TEMP_ERROR) &&
		(previous_temp_value != -220))
	{
		pr_err("%s temp value has got problem suddenly. Current temp value = %d'c, Previous temp value = %d'c \n",
			__func__, (int)results.physical, previous_temp_value);
