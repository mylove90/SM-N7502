...
#ifdef CONFIG_NFC_PN547_CLK_REQ_SW_CTRL
#include <linux/clk.h>
...
#ifdef CONFIG_NFC_PN547_CLK_REQ_SW_CTRL
static void pn547_work_func_clock(struct work_struct *work)
{
	...
	if (gpio_get_value(pn547_dev->clk_req_gpio)) {
		if (pn547_dev->clock_state == false) {
			...
			pr_err("%s:  RF_CLK ON(%d)\n",	__func__, ret);
			if (ret < 0) {
				pr_err("%s:  Failed to vote for RF_CLK2 ON (%d)\n",
						__func__, ret);
			}else{
		...
		if (pn547_dev->clock_state == true) {
			...
			pr_err("%s:  RF_CLK OFF(%d)\n",	__func__, ret);
			if (ret < 0) {
				pr_err("%s:  Failed to vote for RF_CLK2 OFF (%d)\n",
						__func__, ret);
			}else{
...
#if defined(CONFIG_NFC_PN547_CLK_REQ_SW_CTRL) || (CONFIG_NFC_PN547_8226_USE_BBCLK2)
	u32 gpio_flags;
#endif
		       ...
		       const struct i2c_device_id *id)
{
	...
	if (ret)
		goto err_firm;
#ifdef CONFIG_NFC_PN547_CLK_REQ_SW_CTRL
	platform_data->clk_req_irq = gpio_to_irq(platform_data->clk_req_gpio);
	pr_info("[pn547] request_irq(clk_req) failed! clk_req_irq=0x%x, clk_req_gpio=%d\n",platform_data->clk_req_irq,platform_data->clk_req_gpio);

	ret = gpio_request(platform_data->clk_req_gpio, "pn547_clk_req");
	if (ret)
		goto err_clk_req;
#endif
#ifdef CONFIG_NFC_PN547_8226_USE_BBCLK2
...
*/
	...
	INIT_WORK(&pn547_dev->work_nfc_clock, pn547_work_func_clock);
#endif
	...
	pr_err("[pn547] request_irq(clk_req) failed! clk_req_irq=0x%x, clk_req_gpio=%d\n",pn547_dev->clk_req_irq,pn547_dev->clk_req_gpio);
	if (ret) {
