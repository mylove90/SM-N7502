			  ...
			  const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		...
		goto err_input_free_device_proximity;
	}
	...
	if (ret < 0) {
		...
		goto err_input_free_device_light;
	}
...
err_sysfs_create_group_light:
	sensors_delete_symlink(taos->light_input_dev);

err_create_symlink_light:
	input_unregister_device(taos->light_input_dev);
err_input_free_device_light:
...
err_input_free_device_proximity:
	input_free_device(input_dev);
