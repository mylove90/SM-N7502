				...
				struct sec_battery_info *battery)
{
	...
	psy_do_property(battery->pdata->fuelgauge_name, get,
		POWER_SUPPLY_PROP_TEMP_ADC, value);
	battery->temp_adc_bms = value.intval;
#endif
		...
		struct sec_battery_info *battery)
{
	...
	ret = of_property_read_u32(np, "battery,temp_high_threshold_event",
		(unsigned int *)&pdata->temp_high_threshold_event);
	if (ret)
		pr_info("%s: temp_high_threshold_event is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_high_recovery_event",
		(unsigned int *)&pdata->temp_high_recovery_event);
	if (ret)
		pr_info("%s: temp_high_recovery_event is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_low_threshold_event",
		(unsigned int *)&pdata->temp_low_threshold_event);
	if (ret)
		pr_info("%s: temp_low_threshold_event is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_low_recovery_event",
		(unsigned int *)&pdata->temp_low_recovery_event);
	if (ret)
		pr_info("%s: temp_low_recovery_event is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,temp_high_threshold_normal",
		(unsigned int *)&pdata->temp_high_threshold_normal);
	if (ret)
		pr_info("%s: temp_high_threshold_normal is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_high_recovery_normal",
		(unsigned int *)&pdata->temp_high_recovery_normal);
	if (ret)
		pr_info("%s: temp_high_recovery_normal is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_low_threshold_normal",
		(unsigned int *)&pdata->temp_low_threshold_normal);
	if (ret)
		pr_info("%s: temp_low_threshold_normal is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_low_recovery_normal",
		(unsigned int *)&pdata->temp_low_recovery_normal);
	if (ret)
		pr_info("%s: temp_low_recovery_normal is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,temp_high_threshold_lpm",
		(unsigned int *)&pdata->temp_high_threshold_lpm);
	if (ret)
		pr_info("%s: temp_high_threshold_lpm is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_high_recovery_lpm",
		(unsigned int *)&pdata->temp_high_recovery_lpm);
	if (ret)
		pr_info("%s: temp_high_recovery_lpm is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_low_threshold_lpm",
		(unsigned int *)&pdata->temp_low_threshold_lpm);
	if (ret)
		pr_info("%s: temp_low_threshold_lpm is Empty\n", __func__);
	
	ret = of_property_read_u32(np, "battery,temp_low_recovery_lpm",
		(unsigned int *)&pdata->temp_low_recovery_lpm);
	if (ret)
