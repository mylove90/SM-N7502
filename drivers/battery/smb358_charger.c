				...
				struct i2c_client *client)
{
	...
	if (input_value.intval ==
		...
		if ((data & 0x10) && charger->pdata->vbus_ctrl_gpio) {
			...
			/* check top off charging stop */
			smb358_i2c_read(client, SMB358_STATUS_C, &data);
			pr_info("[SMB358] check skip. current (%d) status_c (0x%x)\n",
				chgcurrent, data);

			if ((chgcurrent ==
				smb358_get_fast_charging_current_data(charger->charging_current)) &&
				(data & 0x06)) {
...
static int smb358_charger_parse_dt(struct sec_charger_info *charger)
{
	...
	if (np == NULL) {
		...
		ret = of_property_read_u32(np, "battery,chg_functions_setting",
					&pdata->chg_functions_setting);
		if (ret < 0)
			pr_err("%s: chg_functions_setting read failed (%d)\n", __func__, ret);
