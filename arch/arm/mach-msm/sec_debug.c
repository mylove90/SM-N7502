...
extern void mdm_raise_mdm_errfatal(void);
#endif

void sec_debug_check_crash_key(unsigned int code, int value)
{
#if defined(CONFIG_MACH_BAFFIN2_CHN_CMCC)
	static enum { NONE, STEP1, STEP2, STEP3, STEP4, STEP5, STEP6} state = NONE;
#else
	...
	case NONE:
		if (code == KEY_VOLUMEDOWN && value)
			state = STEP1;
#if defined(CONFIG_MACH_BAFFIN2_CHN_CMCC)
		else if (code == KEY_VOLUMEUP && value)
			state = STEP4;
#endif
		else
			state = NONE;
...
void sec_debug_print_file_list(void)
{
	...
	nCnt=(unsigned int)atomic_read(&files->count);
